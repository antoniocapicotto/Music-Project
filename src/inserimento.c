...
database InserireDatiBrano(database Database, Brano NuovoBrano) {
	...
	} while ((strlen(NuovoBrano.titolo) >= 30)
			|| (strlen(NuovoBrano.titolo) < 1));
...
database InserireDatiAlbum(database Database, Album NuovoAlbum) {
	...
	} while ((strlen(NuovoAlbum.titolo) >= 30)
			|| (strlen(NuovoAlbum.titolo) < 0));
...
database InserireDatiPlaylist(database DataBase, Playlist NuovaPlaylist) {
	...
	} while ((strlen(NuovaPlaylist.nome) >= 30)
			|| (strlen(NuovaPlaylist.nome) < 0));
	...
	if (TrovaIdPlaylist(DataBase, NuovaPlaylist.nome,
			NuovaPlaylist.pubblica) != SENTINELLA) {
		...
		} while ((strlen(NuovaPlaylist.descrizione) >= 250)
				|| (strlen(NuovaPlaylist.descrizione) < 0));
...
database InserireDatiArtista(database Database, Artista NuovoArtista) {
	...
	} while ((strlen(NuovoArtista.nome) >= 30)
			|| (strlen(NuovoArtista.nome) < 0));
	...
	} while ((strlen(NuovoArtista.cognome) >= 30)
			|| (strlen(NuovoArtista.cognome) < 0));
	...
	} while ((strlen(NuovoArtista.nomeArte) >= 30)
			|| (strlen(NuovoArtista.nomeArte) < 0));

	if (TrovaIdArtista(Database, NuovoArtista.nome, NuovoArtista.cognome)
			== 1) {
...
database InserireDatiUtente(database Database, Utente NuovoUtente) {
	...
	} while ((strlen(NuovoUtente.nomeUtente) >= 60)
			|| (strlen(NuovoUtente.nomeUtente) < 6));

	do {

		LeggerePassword(NuovoUtente.password);

	} while ((strlen(NuovoUtente.password) >= 60)
			|| (strlen(NuovoUtente.password) < 6));

	if (TrovaIdUtente(Database, NuovoUtente.nomeUtente,
			NuovoUtente.password) != SENTINELLA) {
