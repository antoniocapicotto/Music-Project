...
database CancellaCollegamentoBranoAlbum( int  IdBrano , database Database ) {
    ...
    i=0;
    while( ( Database.BranoAlbum[i].idBrano != IdBrano ) && ( Database.BranoAlbum[i].idBrano != SENTINELLA ) ) {
...
database CancellaCollegamentoBranoGenere( int IdBrano , database Database ) {
    ...
    i = 0;
    while( ( Database.BranoGenere[ i ].idBrano != IdBrano ) && ( Database.BranoGenere[i].idBrano != SENTINELLA ) ) {
...
database CancellaCollegamentoPlaylistBrano( int IdBrano , database Database ) {
    ...
    i = 0;
    while( ( Database.PlaylistBrano[i].idBrano != IdBrano ) && ( Database.PlaylistBrano[i].idBrano != SENTINELLA ) ) {
...
database CancellaCollegamentoBranoArtista( int  IdBrano , database Database ) {
    ...
    i = 0;
    while( ( Database.BranoArtista[i].idBrano != IdBrano ) && ( Database.BranoArtista[i].idBrano != SENTINELLA ) ){
...
// Funzione che permette di cancellare i collegamenti tra un album con più brani
...
database CancellaCollegamentiAlbumBrani( int IdAlbum , database Database ) {
    ...
    i = 0;
    while( ( Database.BranoAlbum[ i ].idAlbum != IdAlbum ) && ( Database.BranoAlbum[ i ].idBrano != SENTINELLA ) ) {
...
database CancellaCollegamentiArtistaBrani( int IdArtista , database Database ) {
    ...
    i = 0;
    while( ( Database.BranoArtista[ i ].idArtista != IdArtista ) && ( Database.BranoArtista[ i ].idBrano != SENTINELLA ) ) {
...
database CancellaCollegamentiPlaylistBrani( int IdPlaylist , database Database ) {
    ...
    i = 0;
    while( ( Database.PlaylistBrano[ i ].idPlaylist != IdPlaylist ) && ( Database.PlaylistBrano[ i ].idBrano != SENTINELLA ) ) {
...
database CancellaCollegamentiGenereBrani( int IdGenere , database Database ) {
    ...
    i = 0;
    while( ( Database.BranoGenere[ i ].idGenere != IdGenere ) && ( Database.BranoGenere[ i ].idBrano != SENTINELLA ) ) {
